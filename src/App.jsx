...
// GCTU brand logo. Declared at module level: defined inside App it was a new
// component type on every render, so React remounted both logo images each time.
const GctuLogo = () => (
  <img
...
export default function App() {
  ...
  useEffect(() => {
    const goOnline = () => setShowOfflineBanner(false);
    const goOffline = () => setShowOfflineBanner(true);
  ...
  // The effect can run after the window "load" event has already fired (a
  // fast or cached load), in which case a load listener never triggers and
  // the worker silently never registers. Register immediately in that case.
  useEffect(() => {
